...
# load(file="results/Rdata/fig2_sampled_noisy_params.RData")
...
build_stan_data <- function(sim_res,rep_id) {
  sim_res <- sim_res[complete.cases(sim_res),]
  true_val <- sampled_params[[rep_id]]
...
fit_one_repitition <- function(rep_id,rst_models,
  ...
  for (m in 1:length(rst_models)) {
    ...
    for (d in 1:dl){
      ...
      sdata <- build_stan_data(sim_res,rep_id)
  ...
  save(res_list, file=paste0("results/Rdata/fig3_samples_true_fixed/fig3_core_estimate_true_fixed_i",rep_id,".RData"))
}
