...
ggplot(df, aes(x = Ci, y = An,  group = key, shape = Limitation)) +
...
ggplot(df, aes(x = Ci, y = An,
               color = model,
               shape = Limitation)) +
  geom_line(aes(group = key), linewidth = 1) +
  geom_point(size = 2) +
  facet_grid(model~.)+
  theme_bw()+
  scale_shape_manual(
    values = c(Wc = 16, Wj = 17, Wp = 15, Ac = 16, Aj = 17, Ap = 15)  # ● ▲ ■
  )
