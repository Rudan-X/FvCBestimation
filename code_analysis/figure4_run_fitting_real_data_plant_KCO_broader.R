...
ACi <- ACi[ACi$ID%in%plant_id,]
AQ <- AQ[AQ$ID%in%plant_id,]
...
fit_one_repitition <- function(ind_x,rst_models,   # list of compiled Stan models
  ...
  for (m in 1:length(rst_models)) {
    ...
    for (d in 1:2){
      if (d==1){
        curve <- ACi[ACi$ID==p_id,c("Ci","PPFD","Photo")]
        colnames(curve)[3] <- "An"
      }else{
        curve1 <- ACi[ACi$ID==p_id,c("Ci","PPFD","Photo")]
        colnames(curve1)[3] <- "An"
        curve2 <- AQ[AQ$ID==p_id,c("Ci","PAR","A")]
        colnames(curve2)[c(2,3)] <- c("PPFD","An")
        curve <- rbind(curve1,curve2)
      }
      curve$O <- 210

      print(paste0("Plant",ind_x,"_",names(rst_models)[m],"_",datatypes[d]))

      case_name <- paste0("Plant",p_id,"_",names(rst_models)[m],"_",datatypes[d])
  ...
  save(res_list, file=paste0("results/Rdata/fig4_plant_KCO_B/fig4_plant_fit_i",ind_x,".RData"))
}


for (rep_x in 237:354){
  fit_one_repitition(rep_x, rst_models = rst_models)
}



for (rep_x in 119:236){
  fit_one_repitition(rep_x, rst_models = rst_models)
}
