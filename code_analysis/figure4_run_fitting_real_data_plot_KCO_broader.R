...
fit_one_repitition <- function(ind_x,rst_models,   # list of compiled Stan models
  ...
  res_list <- list()
  plot_id <- common_plot[ind_x]
  for (m in 1:length(rst_models)) {
    stan_m <- rst_models[[m]]
    for (d in 1:2){
      if (d==1){
        curve <- ACi[ACi$Plot==plot_id,c("Ci","PPFD","Photo")]
        colnames(curve)[3] <- "An"
      }else{
      ...
      print(paste0("Plot",ind_x,"_",names(rst_models)[m],"_",datatypes[d]))

      case_name <- paste0("Plot",ind_x,"_",names(rst_models)[m],"_",datatypes[d])
      ...
      if (length(rstan::extract(fit, permuted = FALSE)) == 0) {
        ...
        keep <- c(keep, "sigma", "log_lik_sum", "sigma","lp__",".draw")
  ...
  save(res_list, file=paste0("results/Rdata/fig4_plot_KCO_B/fig4_plot_KCO_i",ind_x,".RData"))
}
...
for (rep_x in 105:130){
  fit_one_repitition(rep_x, rst_models = rst_models)
}



for (rep_x in 131:156){
  fit_one_repitition(rep_x, rst_models = rst_models)
}
