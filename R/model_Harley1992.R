...
Harley1992 = function(envs,pars,quadratic=TRUE, Ct=NULL) {
  ret = list()
  ...
  if (quadratic){
    ...
    ret$Ac <- pars$V_cmax * envs$C_chl / (envs$C_chl + pars$K_CO) * (1 - pars$gamma_star/envs$C_chl) - pars$R_d
    ret$Aj <- J * (envs$C_chl -  pars$gamma_star) / (4 * (envs$C_chl + 2 * pars$gamma_star)) - pars$R_d
  }
