...
functions {
  ...
  real fvcb_model(real Ci,int has_Cc, real Cc, real O, real PPFD,
...
model {
  ...
  // Likelihood
  for (n in 1:N) {
    A_hat[n] = fvcb_model(Ci[n], has_Cc, Cc[n], O[n], PPFD[n],
                          V_cmax, J_max, R_d,
                          K_C, K_O, gamma_star,
                          alpha_J, theta_J);
  }
...
generated quantities {
  ...
  for (n in 1:N) {
    real A_hat_n;
    A_hat_n = fvcb_model(Ci[n], has_Cc, Cc[n], O[n], PPFD[n],
                              V_cmax, J_max, R_d,
                              K_CO, gamma_star,
                              alpha_J, theta_J);
